...
**URL:** `POST https://rosa-mexicano-vouchers-production.up.railway.app/api/webhook`
...
2. ✅ URL está correta? (https://rosa-mexicano-vouchers-production.up.railway.app/api/webhook)
3. ✅ Eventos `PAYMENT_CONFIRMED` e `PAYMENT_RECEIVED` estão ativados?
4. ✅ Backend está rodando? (Verifique `/health`)

**Teste manual:**
```bash
curl -X POST "https://rosa-mexicano-vouchers-production.up.railway.app/api/webhook" \
...
**Exemplo:**
```bash
curl -X POST "https://rosa-mexicano-vouchers-production.up.railway.app/api/process-payment-manually" \
  -H "Content-Type: application/json" \
...
1. Clique em `rosa-mexicano-vouchers`
2. Vá na aba **"Logs"**
