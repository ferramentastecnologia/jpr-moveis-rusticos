/**
 * Backend Sistema de Vouchers - Rosa Mexicano
 *
 * Workflow:
        ...
        try {
            ...
            try {
                ...
                if (fs.existsSync(logoPath)) {
                    ...
                    doc.fontSize(32).font('Helvetica-Bold').fillColor('#FFFFFF').text('Rosa Mexicano', 0, 95, { align: 'center' });
                }
            } catch (logoError) {
                doc.fontSize(60).fillColor('#FFFFFF').text('🌮', 0, 30, { align: 'center' });
                doc.fontSize(32).font('Helvetica-Bold').fillColor('#FFFFFF').text('Rosa Mexicano', 0, 95, { align: 'center' });
            ...
            // Footer
            const footerY = 700;
            doc.fontSize(9).fillColor('#666666').text('Rosa Mexicano Blumenau', 0, footerY, { align: 'center' });
...
async function generateVoucherPDF(voucherData) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Logo do Rosa Mexicano centralizado
            try {
                const logoPath = path.join(__dirname, 'images', 'logo.png');
                if (fs.existsSync(logoPath)) {
...
async function sendVoucherEmail(voucherData, pdfPath) {
    try {
        ...
        const mailOptions = {
            ...
            subject: `🌮 Seu Voucher Rosa Mexicano - ${voucherData.code}`,
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <div style="background: linear-gradient(135deg, #E91E63, #9C27B0); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
                        <h1 style="margin: 0;">🌮 Rosa Mexicano</h1>
                    ...
                    <div style="text-align: center; padding: 20px; color: #666; font-size: 12px;">
                        <p>© ${new Date().getFullYear()} Rosa Mexicano Blumenau</p>
                    </div>
                </div>
            `,
            attachments: [
...
app.post('/api/create-payment', async (req, res) => {
    try {
        ...
        const paymentData = {
            ...
            description: `${quantity}x ${voucherName} - Rosa Mexicano Blumenau`,
...
app.listen(CONFIG.PORT, () => {
    console.log(`
╔════════════════════════════════════════════════╗
║   🌮 Sistema de Vouchers - Rosa Mexicano      ║
